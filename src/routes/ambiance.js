...
const { buildNow, buildQuietHours, buildHistory } = require('../utils/ambiance');

const router = express.Router();

async function ensureLocation(slug) {
...
router.get('/compare', async (req, res, next) => {
  try {
    ...
    const windowStr = req.query.window || '30m';
    ...
    const quietest = ranked.length ? ranked.reduce((a, b) => (b.score.noise < a.score.noise ? b : a)).location : null;
    const busiest = ranked.length ? ranked.reduce((a, b) => (b.score.noise > a.score.noise ? b : a)).location : null;
    success(res, 200, { window: windowStr, locations: results, quietest, busiest });
  } catch (e) { next(e); }
});

// GET /v1/ambiance/:locationSlug/now?window=30m
router.get('/:locationSlug/now', async (req, res, next) => {
  try {
    ...
    const since = new Date(Date.now() - parseDuration(windowStr));
...
router.get('/:locationSlug/quiet-hours', async (req, res, next) => {
  try {
    await ensureLocation(req.params.locationSlug);
...
router.get('/:locationSlug/history', async (req, res, next) => {
  try {
    await ensureLocation(req.params.locationSlug);
    ...
    success(res, 200, { location: slug, bucket: bucketStr, series: buildHistory(measurements, parseDuration(bucketStr)) });
  } catch (e) { next(e); }
});
